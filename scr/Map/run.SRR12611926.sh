...
do
  #cd ${PREFIX}
  Fastq_R1=${PREFIX}_1.fastq.gz
  Fastq_R2=${PREFIX}_2.fastq.gz
## mapping
...
## read bam convered to bed and calculate coverage
  ...
  cat ${PREFIX}.bedgraph | sort -k1,1 -k2,2n > ${PREFIX}.sorted.bedgraph
  ${bedGraphToBigWig} ${PREFIX}.sorted.bedgraph ${chrSize} > ${PREFIX}.bigwig
